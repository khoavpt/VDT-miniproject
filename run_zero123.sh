...
FINAL_OUTPUT_DIR="/home/khoahd/vdt_phase1/outputs/zero123_finals"
...
for png_file in $png_files; do
    ...
    CUDA_VISIBLE_DEVICES=6 python launch.py --config configs/zero123.yaml --train --gpu 0 data.image_path="$IMG_DIR/${filename}.png"

    echo "Finding output folder..."
    echo "DEBUG: Listing all output directories:"
    ls -l threestudio/outputs/zero123

    echo "DEBUG: Searching for pattern: *${filename}.png@*"
    folder=$(find /home/khoahd/vdt_phase1/threestudio/outputs/zero123 -maxdepth 1 -type d -name "*${filename}.png@*" | sort | tail -n 1)
    ...
    CUDA_VISIBLE_DEVICES=6 python launch.py --config configs/zero123.yaml --export --gpu 0 resume="${folder}/ckpts/last.ckpt" system.exporter_type=mesh-exporter

    echo "Copying mesh model..."
    EXPORT_ROOT="/home/khoahd/vdt_phase1/threestudio/outputs/zero123"
    ...
    # Try to find model.obj in it800-export, it700-export, or it600-export
    obj_path=""
    for export_iter in it800-export it700-export it600-export; do
        candidate_path="${export_folder}/save/${export_iter}/model.obj"
        if [ -f "$candidate_path" ]; then
            obj_path="$candidate_path"
            break
        fi
    done

    if [ -z "$obj_path" ]; then
        echo "Error: model.obj not found in it800-export, it700-export, or it600-export"
        exit 1
    fi
